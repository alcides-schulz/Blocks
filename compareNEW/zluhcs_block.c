...
//#define VERSION 641 increase line penalty begin
...
void init_player(PLAYER *player) {
	...
	player->eval.param.lines[0] = -1.500000;              player->eval.param.lines[1] = 2.000000;
	...
	player->eval.param.block[0] = -0.500000;             player->eval.param.block[1] = -0.100000;
...
double evaluate(PLAYER *player, int prev_removed, int prev_row) {
			...
			for (holes_row = field_row + 1; holes_row < game_settings.field_height; holes_row++) {
				...
				if (player->field[holes_row * game_settings.field_width + field_col] == CELL_EMPTY) {
					...
					if (is_blocked(player->field, holes_row, field_col)) {
						...
						//block_holes += count_block_holes(player->field, holes_row, field_col);
	...
	for (phase = 0; phase < 2; phase++) {
		...
		score[phase] += player->eval.param.block[phase] * block_holes;
	...
	total_phase = game_settings.field_height;

	game_phase = row_first_solid;



	final_score = ((score[0] * game_phase) + (score[1] * (total_phase - game_phase))) / total_phase;
...
void find_best_move(PLAYER *player) {
	...
	if (game_state.this_piece_type == 'T'/* && player->t_spin_allow*/) {
		...
		if (get_t_spin_data(player->field, &t_spin) /*&& t_spin.lines*/) {
	...
	for (i = 0; i < this_moves.count; i++) {
		...
		if (game_state.next_piece_type == 'T' && player->four_line_enabled == 0) {
		...
		for (j = 0; j < next_moves.count; j++) {
			...
			if (player->four_line_enabled)

				score += four_line_eval(player->field);



			//if (this_moves.moves[i].row == 12 && this_moves.moves[i].col == -1) {

			//            print_player(player);

			//            printf("eval: %f best: %f 4line: %d\n", score, player->move.score, player->four_line_enabled);
...
void view(void) {
	...
	FILE    *f = fopen("c:/alcides/block/positions.txt", "r");
...
void optimize(PLAYER *player, double initial_values[PARAM_COUNT]) {
	...
	for (x = 0; x < PARAM_COUNT; x++) {
		...
		//player->eval.param.heigh[0] = -1.755;           player->eval.param.heigh[1] = -1.029;

		//player->eval.param.tslin    =  0.729; player->eval.param.tspct    =  6.561;



		if (!strcmp(PARAM_LABEL[x], "tslin") || !strcmp(PARAM_LABEL[x], "tspct"))

			printf("    player->eval.param.%s    = %s%3.6f;\n",

				PARAM_LABEL[x], (best_values[x] >= 0 && best_values[x] < 10 ? " " : ""), best_values[x]);

		else

			printf("    player->eval.param.%s[%d] = %s%3.6f;\n",

				PARAM_LABEL[x], (x % 2 == 0 ? 0 : 1), (best_values[x] >= 0 && best_values[x] < 10 ? " " : ""), best_values[x]);
