...
void init_player(PLAYER *player) {
    ...
    //player->eval.param.tsbon =  9.000000;
    //player->eval.param.tsrev =  2.000000;
    //player->eval.param.tspre =  5.000000;
    //player->eval.param.occup =  1.000000;
    //player->eval.param.avrow = 10.000000;
...
double evaluate(PLAYER *player, int prev_removed, int prev_row) {
    ...
    if (avg_heights < player->eval.param.avrow) {
        if (get_t_spin_data(player->field, &t_spin)) {
            if (t_spin.row >= 12 && t_spin.percent_complete == 1.0) {
