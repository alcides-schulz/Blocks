...
double four_line_eval(char *field);
...
void init_player(PLAYER *player) {
	...
	//player->eval.param.lines[0] =  0.650000;
	//player->eval.param.lines[1] = 2.000000;
	//player->eval.param.lines[1] =  2.200000;
	
	player->eval.param.lines[0] = -2.000000;
	player->eval.param.lines[1] =  2.000000;
	...
	player->eval.param.block[0] = -0.100000;
	player->eval.param.block[1] = -0.100000;
...
int t_spin_block_count(char *field, int row, int from_col, int to_col) {
	int     c;
	int		blocks = 0;
...
double t_spin_evaluate(char *field, T_SPIN *t_spin) {
	double	score = 0;
	//int		row;
	...
	//if (t_spin_block_count(field, t_spin->row + 1, 0, t_spin->col))
	//	return 0;
	//if (t_spin_block_count(field, t_spin->row + 1, t_spin->col + T_SLOT_COL, game_settings.field_width))
	//	return 0;
	//if (t_spin_block_count(field, t_spin->row + 2, 0, t_spin->col))
	//	return 0;
	//if (t_spin_block_count(field, t_spin->row + 2, t_spin->col + T_SLOT_COL, game_settings.field_width))
	//	return 0;

	//score -= 5 * t_spin_block_count(field, t_spin->row - 2, 0, game_settings.field_width);
	//score -= 5 * t_spin_block_count(field, t_spin->row - 1, 0, game_settings.field_width);
	//score -= 5 * t_spin_block_count(field, t_spin->row - 0, 0, game_settings.field_width);

	//for (row = t_spin->row + 3; row < game_settings.field_height; row++)
	//	score -= 5 * t_spin_block_count(field, row, 0, game_settings.field_width);

	score += 100;
	score += t_spin->percent_complete * 10;
	score += t_spin->row * 10;
	...
	return score;
			...
			//		t_spin->row1filled = row1_filled;
			//		t_spin->row2filled = row2_filled;
...
int get_t_spin_data(char *field, T_SPIN *t_spin) {
	...
	for (field_row = 0; field_row < game_settings.field_height - T_SLOT_ROW; field_row++) {
		for (field_col = 0; field_col < game_settings.field_width - T_SLOT_COL; field_col++) {
			if (field_col > 0 && t_slot_match(field, T_SLOT_LEFT, field_row, field_col)) {
				if (t_slot_drop(field, field_row, field_col)) {
					t_spin->row = field_row;
					t_spin->col = field_col;
					t_spin->type = T_SPIN_LEFT;
					t_spin->row1filled = is_filled(field, field_row + 1, 0, field_col) && is_filled(field, field_row + 1, field_col + T_SLOT_COL, game_settings.field_width);
					t_spin->row2filled = is_filled(field, field_row + 2, 0, field_col) && is_filled(field, field_row + 2, field_col + T_SLOT_COL, game_settings.field_width);
					t_spin->percent_complete = calc_t_spin_complete(field, field_row + 1, field_row + 2);
					return 1;
				}
			}
			if (field_col < game_settings.field_width - 1 && t_slot_match(field, T_SLOT_RIGHT, field_row, field_col)) {
				if (t_slot_drop(field, field_row, field_col)) {
					t_spin->row = field_row;
					t_spin->col = field_col;
					t_spin->type = T_SPIN_RIGHT;
...
/*
	- no more than 2 solid lines

*/
int can_search_four_line(char *field) {
	int		bottom_row;
	int		block_count = 0;
	int		block_row;

	bottom_row = get_bottom_row(field);
	if (bottom_row < 19 || get_highest_column(field, bottom_row) > 10)
		return 0;
	for (block_row = bottom_row; block_row > bottom_row - 4; block_row--) {
		if (field[block_row * game_settings.field_width] != CELL_EMPTY)
			block_count++;
	}
	if (block_count > 1)
		return 0;
...
double four_line_eval(char *field) {
	...
	bottom_row = get_bottom_row(field);
	for (row = bottom_row; row > bottom_row - 4; row--) {
		if (field[row * game_settings.field_width] != CELL_EMPTY)
			value -= 100.0;
		for (col = 1; col < game_settings.field_width; col++) {
			if (field[row * game_settings.field_width + col] != CELL_EMPTY)
				value += 1.0;
...
void find_best_move(PLAYER *player) {
	...
	player->four_line_enabled = can_search_four_line(player->field);
	player->four_line_enabled = 0;
	player->top.count = 0;
	...
	for (i = 0; i < this_moves.count; i++) {
		...
		if (game_state.this_piece_type == 'I' && filled_lines > 1) {
			player->move.score = MY_MAXVAL;
		...
		if (game_state.next_piece_type == 'T') {
			...
			if (get_t_spin_data(player->field, &t_spin)) {
                ...
                score += player->eval.param.tslin;
                score += player->eval.param.tslin;
                score += player->eval.param.tspct * t_spin.percent_complete;
