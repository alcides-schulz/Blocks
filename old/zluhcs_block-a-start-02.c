...
int path_fits(char *field, PIECE *piece, int row, int col) {
	int     pc_row;
	int     pc_col;
	int		bk_row;
	int		bk_col;
...
char	*possible[27] = { "d", "Rd", "RRd", "RRRd", "Ld", "LLd", "LLLd", "l", "Rl", "RRl", "RRRl", "Ll", "LLl", "LLLl", "r", "Rr", "RRr", "RRRr", "Lr", "LLr", "LLLr", "dR", "dL", "lR", "lL", "rR", "rL" };

void init_path(PATH *path, char *field, PIECE *piece, int to_row, int to_col) {
	int		i;
	int		j;
	PIECE	work;
	int		use;
	...
	for (i = 0; i < 27; i++) {
		use = 1;
		memcpy(&work, piece, sizeof(PIECE));
		for (j = 0; use && possible[i][j]; j++) {
			switch (possible[i][j])
			{
			case PATH_DOWN: 
				if (!path_fits(field, &work, work.row + 1, work.col))
					use = 0;
				work.row++;
				break;
			case PATH_RIGHT:
				if (!path_fits(field, &work, work.row, work.col + 1))
					use = 0;
				work.col++;
				break;
			case PATH_LEFT:
			...
			case PATH_TURNR:
				rotate_right(&work);
				if (!path_fits(field, &work, work.row, work.col))
					use = 0;
				break;
			case PATH_TURNL:
				rotate_left(&work);
		...
		if (use) {
			path->possible[path->count++] = possible[i];
		}
	}
	// if row < first block - shape_size return

}

int find_path(char *field, PIECE *piece, int to_row, int to_col) {
	PIECE	work;
	PATH	path[100];
	int		index = 0;
	...
	while (work.row != to_row && work.col != to_col) {
		init_path(&path[index], field, &work, to_row, to_col);
...
void print_best_moves(PLAYER *player) {
	...
	for (i = 0; i < player->top.count; i++) {
		...
		place_piece(player->field, &piece, player->top.moves[i].row, player->top.moves[i].col, piece.type);
		print_player(player);
