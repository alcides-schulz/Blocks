	...
	for (field_row = 0; field_row <= game_settings.field_height - T_SLOT_ROW; field_row++) {
		for (field_col = 0; field_col <= game_settings.field_width - T_SLOT_COL; field_col++) {
			if (t_slot_match(field, T_SLOT_LEFT, field_row, field_col)) {
                ...
                percent = calc_t_spin_complete(field, field_row + 1, field_row + 2);
				if (/*row1_filled ||*/ row2_filled && percent >= t_spin->percent_complete) {
...
double evaluate(PLAYER *player, int prev_removed, int prev_row) {
    ...
    if (row_first_solid > 15/*player->ts_solid*/ /*&& holes <= player->ts_holes*/) {
        if (get_t_spin_data(player->field, &t_spin)) {
            if (t_spin.row >= player->ts_mnrow && t_spin.percent_complete == 1.0) {
                final_score += player->ts_bonus * t_spin.percent_complete;
                final_score += (player->eval.param.lines * player->ts_lines * lines) + (holes * player->eval.param.holes);
            }
            else {
                if (holes <= 0 && get_t_spin_init(player->field, &t_spin) && t_spin.row >= player->ts_mnrow) {
                    final_score += (200.0 * t_spin.percent_complete) + (holes * player->eval.param.holes) + (2.0 * lines);
                    //print_player(player);
                    //printf("(%d, %d) %f %d\n", t_spin.row, t_spin.col, t_spin.percent_complete, holes);
                    //pause("init");
                }
            }
        }
