...
void init_player(PLAYER *player) {
	...
	player->eval.param.afull = 3.0; player->eval.param.aline = 1;
}
...
void find_best_move(PLAYER *player) {
	...
	if (almost_full_count(player->field) > player->eval.param.afull)
		player->eval.param.lines[0] = player->eval.param.aline;
	else
		player->eval.param.lines[0] = -2;
...
double get_points(PLAYER *player, double param[PARAM_COUNT]) {
	...
	return simulate_game(player, 0, 100, 0);
}
